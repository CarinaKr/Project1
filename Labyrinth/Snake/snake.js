...
function loop()
{
	...
	zMainCtx.fillStyle="white";
		...
		zMainCtx.fillText("Points: "+zPunkte,20,20);
	...
	if(!zGameOver&&zWarteZeit==0)
	{
		...
		else if(zRichtung==3) //bewege nach link
		{
			for(var i=zIndexKoerper-1;i>=1;i--)
			{
				hatKoerper[i].zX=hatKoerper[i-1].zX;
		...
		if(hatKopf.zX==hatFutter.zX&&hatKopf.zY==hatFutter.zY)
		{
			zPunkte++;
			hatFutter.zX=Math.round(Math.random()*zXFelder);
			hatFutter.zY=Math.round(Math.random()*zYFelder);
		...
		if(hatKopf.zX<1||hatKopf.zX>zXFelder-2||hatKopf.zY<1||hatKopf.zY>zYFelder-2)
		{
			zGameOver=true;
		}
		for(var j=0;j<zIndexKoerper;j++)
		{
			if(hatKopf.zX==hatKoerper[j].zX&&hatKopf.zY==hatKoerper[j].zY)
			{zGameOver=true;}
		}
		zWarteZeit=zZeitStart;
	}
